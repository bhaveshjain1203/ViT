...
'''
    1.ImageToPatches returns multiple flattened square patches from an input image tensor.
    ...
    2.The PatchEmbedding layer takes multiple image patches in (B,T,Cin) format and returns the embedded patches in (B,T,Cout) format.
'''
...
class ImageToPatches(nn.Module):
    def __init__(self, image_size, patch_size):
        ...
        ''' nn.Unfold module is used to extract patches from the input image tensor. 
            It takes two parameters: kernel_size and stride. 
            In this case, kernel_size is set to patch_size, 
            which means it will extract square patches of size patch_size from the input image. 
            The stride parameter is also set to patch_size, which means the patches will not overlap.
        '''
    ...
    def forward(self, x):
        # assert len(x.size()) == 4 # checking if input has 4 dimensions
        y = self.unfold(x)
        # y obtained will be (N,C,T) -> (N,T,C)
        y = y.permute(0, 2, 1)
        return y
...
class VisionTransformerInput(nn.Module):
    def __init__(self, image_size, patch_size, in_channels, embed_size):
        ...
        # self.image_to_patches = ImageToPatches(image_size, patch_size)
        # self.patch_embedding = PatchEmbedding(patch_size * patch_size * in_channels, embed_size)
        
        # POSITION EMBEDDING
        self.num_patches = (self.image_size // self.patch_size) ** 2
        self.position_embed = nn.Parameter(torch.randn(self.num_patches, self.embed_size)) # position_embed is the learned embedding for the position of each patch in the input image. They correspond to the cosine similarity of embeddings
...
class VisionTransformerForSegmentation(nn.Module):
    def __init__(self, image_size, patch_size, in_channels, out_channels, embed_size, num_blocks, num_heads, dropout):
        ...
        for i in range(num_blocks):
            heads.append(SelfAttentionEncoderBlock(embed_size, num_heads, dropout))
